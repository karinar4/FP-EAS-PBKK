...
func (h *ProductTransactionHandler) Routes(prefix string) {
	...
	product_transaction.Use(middleware.AuthenticateJWT())
	{
		...
		product_transaction.GET("/:transaction_id", h.GetProductTransactionsByID)
...
func (h *ProductTransactionHandler) GetProductTransactionsByID(c *gin.Context) {
	...
	res, err := h.productTransactionUseCase.GetProductTransactionsByID(id)
	if err != nil {
